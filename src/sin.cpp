...
class CentralOctaveLookUpTable{
    ...
    void sineTable(int32_t tableHz[], uint8_t keyIndex){
            uint8_t tableSize = sizeof(tableHz) / sizeof(int32_t);
            for(int i=0; i<tableSize;i++){
                tableHz[i] = 1000*sin(2*M_PI*((baseHz[keyIndex])/22000.0)*i);
            }
        }
    ...
    public:
        ...
        uint32_t accessTable(uint32_t keyIndex, uint8_t timeStepIndex){
            switch (keyIndex)
            {
                case(0):
